...
function CardDetail({clickedInfo, removeCard, updateCard}: ICardDetailProps) {
    ...
    useEffect(() => {
        ...
        setNewDescription(() => {return clickedInfo.card.description})
        setNewDeadline(() => {return new Date(clickedInfo.card.deadline)})
        setNewPriority(() => {return clickedInfo.card.priority})
        ...
        <div id="card-detail-background" onClick={e => switchOffDetail(e)}>
            <div id="card-detail" onClick={(e) => {e.stopPropagation()}}>
                <form className="card-detail-container" onSubmit={e => handleSubmit(e)}>
                    ...
                    <input name="date" type="date" value={Moment(newDeadline).format("YYYY-MM-DD")} onChange={(e) => setNewDeadline(new Date(e.target.value))}></input>
                    <div className="card-detail-title">priority: </div>
                    <input name="priority" type="number" min="1" max="5" value={newPriority || 1} onChange={(e) => setNewPriority(parseInt(e.target.value))}></input>
                    <div className="card-detail-inline" style={{marginTop: "18px"}}>
                        ...
                        <button className="cancel-button" onClick={e => switchOffDetail(e)}>Cancel</button>
                    </div>
                </form>
            </div>
        </div>
